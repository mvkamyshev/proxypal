...
export function GeminiKeysTab(props: GeminiKeysTabProps) {
  ...
  const handleAddGeminiKey = async () => {
    ...
    try {
      ...
      toastStore.success(t("apiKeys.toasts.apiKeyAdded", { provider: "Gemini" }));
    } catch (error) {
  ...
  const handleDeleteGeminiKey = async (index: number) => {
    ...
    try {
      ...
      await setGeminiApiKeys(updated);
      setGeminiKeys(updated);
      toastStore.success(t("apiKeys.toasts.apiKeyDeleted", { provider: "Gemini" }));
    } catch (error) {
          ...
          <For each={geminiKeys()}>
            {(key, index) => (
              <div class="flex items-center justify-between rounded-lg border border-gray-200 bg-gray-50 p-3 dark:border-gray-700 dark:bg-gray-800/50">
                ...
                <Button onClick={() => handleDeleteGeminiKey(index())} size="sm" variant="ghost">
                  <svg
