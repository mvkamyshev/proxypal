...
export function AdvancedSettings(props: AdvancedSettingsProps) {
  ...
  const [codexWarmupReport, setCodexWarmupReport] = createSignal<CodexWarmupReport | null>(null);
  const [runningCodexWarmup, setRunningCodexWarmup] = createSignal(false);
  ...
  createEffect(async () => {
    try {
      const report = await getCodexWarmupReport();
      setCodexWarmupReport(report);
    } catch {
      setCodexWarmupReport(null);
    }
  });

  onMount(() => {
    void onCodexWarmupFinished((report) => {
      setCodexWarmupReport(report);
      setRunningCodexWarmup(false);
    })
      .then((unlisten) => onCleanup(unlisten))
      .catch((error) => {
        console.error("Failed to subscribe to codex warmup events:", error);
      });
  });
  ...
  const handleRunCodexWarmup = async () => {
    setRunningCodexWarmup(true);
    try {
      const report = await runCodexWarmup();
      setCodexWarmupReport(report);
      toastStore.success(
        `Codex warmup complete: ${report.warmed}/${report.total} warmed, ${report.failed} failed`,
      );
    } catch (error) {
      toastStore.error("Failed to run Codex warmup", String(error));
    } finally {
      setRunningCodexWarmup(false);
    }
  };
    ...
    <div class="space-y-4">
      <div class="space-y-4">
        ...
        <div class="space-y-4 rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-800/50">
          ...
          <Switch
            checked={props.config().codexWarmupEnabled ?? true}
            description="Refresh every Codex account daily and store warmup/quota results"
            label="Daily Codex Warmup"
            onChange={(checked) => props.handleConfigChange("codexWarmupEnabled", checked)}
          />

          <Show when={props.config().codexWarmupEnabled ?? true}>
            <div class="flex items-center justify-between gap-4">
              <div class="flex-1">
                ...
                <p class="text-xs text-gray-500 dark:text-gray-400">
                  Runs once per day at local time
                </p>
              </div>
              <input
                class="rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-sm focus:border-transparent focus:ring-2 focus:ring-brand-500 dark:border-gray-600 dark:bg-gray-900"
                onChange={(e) => props.handleConfigChange("codexWarmupTime", e.currentTarget.value)}
                type="time"
                value={props.config().codexWarmupTime || "12:00"}
              />
            </div>
            ...
            <div class="rounded-lg border border-gray-200 bg-white p-3 dark:border-gray-700 dark:bg-gray-900/50">
              <div class="flex items-center justify-between gap-3">
                <div>
                  ...
                  <Show when={codexWarmupReport()} fallback={<p class="text-xs text-gray-500 dark:text-gray-400">No warmup report yet</p>}>
                    {(report) => (
                      <div class="space-y-1 text-xs text-gray-500 dark:text-gray-400">
                        <p>Last run: {new Date(report().generatedAt).toLocaleString()}</p>
                        <p>
                          Warmed {report().warmed}/{report().total}, failed {report().failed}
                        </p>
                      </div>
                    )}
                  </Show>
                </div>
                <Button disabled={runningCodexWarmup()} onClick={handleRunCodexWarmup} variant="secondary">
                  {runningCodexWarmup() ? "Running..." : "Run Now"}
                </Button>
              </div>
            </div>
          </Show>
