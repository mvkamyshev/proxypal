...
function AgentCard(props: AgentCardProps) {
	...
	const statusText = () => {
		if (props.agent.configured) return t("agentSetup.status.configured");
		if (props.agent.installed) return t("agentSetup.status.installed");
						...
						<Show when={props.result.shellConfig}>
							<div class="space-y-2">
								<div class="flex items-center justify-between">
									...
									>
										{copied()
											? t("agentSetup.configModal.copied")
											: t("common.copy")}
									</button>
								</div>
...
export function AgentSetup() {
	...
	const handleConfigure = async (agentId: string) => {
		...
		try {
			...
			toastStore.error(
				t("agentSetup.toasts.configurationFailed"),
				String(error),
			);
		} finally {
			setConfiguring(null);
	...
	const handleApplyEnv = async () => {
		...
		try {
			...
			toastStore.error(
				t("agentSetup.toasts.failedToUpdateShellProfile"),
				String(error),
			);
		}
	};
			...
			<Show when={!proxyStatus().running}>
				<div class="p-3 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800">
					<div class="flex items-center gap-2 text-amber-700 dark:text-amber-300">
						...
						<span class="text-sm">
							{t("agentSetup.startProxyToConfigureAgents")}
						</span>
					</div>
				</div>
			</Show>
