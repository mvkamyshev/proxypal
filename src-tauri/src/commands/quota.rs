...
pub async fn fetch_codex_quota(state: State<'_, AppState>) -> Result<Vec<crate::types::CodexQuotaResult>, String> {
    ...
    let warm_report = load_codex_warm_report().map(|report| {
        report
            .results
            .into_iter()
            .map(|entry| {
                (
                    entry.file.clone(),
                    CodexWarmReportEntryWithGeneratedAt {
    ...
    struct CodexCred {
        ...
        warm_status: Option<String>,
        warm_error: Option<String>,
        last_warmup_at: Option<String>,
        is_active: bool,
    }
    ...
    for entry in entries.flatten() {
        ...
        let is_codex_json = filename.starts_with("codex-") && filename.ends_with(".json");
        let is_codex_disabled = filename.starts_with("codex-") && filename.ends_with(".json.disabled");
        if is_codex_json || is_codex_disabled {
            let file_path = entry.path();
            let warm_info = warm_report
                .as_ref()
                .and_then(|report| report.get(&filename).cloned());
            ...
            let subscription_active_start = auth_claims
                .and_then(|claims| claims.get("chatgpt_subscription_active_start"))
                .and_then(|v| v.as_str())
                .map(|s| s.to_string());
            let subscription_active_until = auth_claims
                ...
                .and_then(|v| v.as_str())
                .map(|s| s.to_string());
            let subscription_last_checked = auth_claims
                .and_then(|claims| claims.get("chatgpt_subscription_last_checked"))
                .and_then(|v| v.as_str())
                .map(|s| s.to_string());
            ...
            match response {
                Ok(resp) => {
                    if resp.status().is_success() {
                        ...
                        let credits_balance = credits["balance"]
                            .as_f64()
                            .or_else(|| credits["balance"].as_str().and_then(|s| s.parse::<f64>().ok()));
                        ...
                        crate::types::CodexQuotaResult {
                            ...
                            subscription_active_start: cred.subscription_active_start,
                            subscription_active_until: cred.subscription_active_until,
                            subscription_last_checked: cred.subscription_last_checked,
                            last_warmup_at: cred.last_warmup_at,
                            warm_status: cred.warm_status,
                            warm_error: cred.warm_error,
