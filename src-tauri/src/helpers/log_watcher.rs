...
fn extract_codex_auth_id(line: &str) -> Option<String> {
    static AUTH_REGEX: std::sync::OnceLock<regex::Regex> = std::sync::OnceLock::new();
    let regex = AUTH_REGEX.get_or_init(|| {
        regex::Regex::new(r"Auth:\s+provider=codex,\s+auth_id=([^,]+),").unwrap()
    });
    regex
        .captures(line)
    ...
    std::thread::spawn(move || {
        ...
        while running.load(Ordering::SeqCst) {
            ...
            while reader.read_line(&mut line).unwrap_or(0) > 0 {
                if let Some(auth_id) = extract_codex_auth_id(&line) {
                    let state = app_handle.state::<crate::state::AppState>();
                    let mut current = state.current_codex_auth.lock().unwrap();
                    if current.as_deref() != Some(auth_id.as_str()) {
                        *current = Some(auth_id.clone());
                        let _ = app_handle.emit("codex-active-auth-changed", auth_id);
                    }
                }
