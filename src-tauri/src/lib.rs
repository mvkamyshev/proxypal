...
pub fn run() {
    ...
    let app_state = AppState {
        ...
        current_codex_auth: Mutex::new(None),
        codex_warmup_running: Arc::new(AtomicBool::new(false)),
    };
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            // Built-in Codex daily warmup scheduler
            let app_handle = app.handle().clone();
            tauri::async_runtime::spawn(async move {
                loop {
                    if crate::helpers::warmup::should_run_daily_codex_warmup() {
                        let state = app_handle.state::<AppState>();
                        match crate::helpers::warmup::run_codex_warmup_job(
                            state.codex_warmup_running.clone(),
                        )
                        .await
                        {
                            Ok(report) => {
                                let _ = app_handle.emit("codex-warmup-finished", report);
                            }
                            Err(error) => {
                                eprintln!("[ProxyPal] Codex warmup failed: {}", error);
                            }
                        }
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::warmup::get_codex_warmup_report,
            commands::warmup::run_codex_warmup,
